...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($email)) {
        ...
        // Enforce constraint: Cannot change your own role
    ...
    <div class="flex flex-col md:flex-row gap-8">
        ...
        <div class="flex-1">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-3xl font-extrabold text-gray-900">Edit User #<?php echo htmlspecialchars($user['id']); ?></h1>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8">
                ...
                <form method="POST" class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" required class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-accent" value="<?php echo htmlspecialchars($user['name']); ?>">
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                        <input type="email" name="email" required class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-accent" value="<?php echo htmlspecialchars($user['email']); ?>">
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Role</label>
                        <select name="role" required class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-accent bg-white" <?php echo($id == $_SESSION['user_id']) ? 'disabled' : ''; ?>>
