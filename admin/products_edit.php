...
$product_tags_q = $pdo->prepare("SELECT tag_id FROM product_tags WHERE product_id = ?");
$product_tags_q->execute([$id]);
$current_tags = $product_tags_q->fetchAll(PDO::FETCH_COLUMN);
                        ...
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <select name="tags[]" multiple class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-accent bg-white h-32">
                                <?php foreach ($tags as $tag): ?>
                                    <option value="<?php echo $tag['id']; ?>" <?php echo in_array($tag['id'], $current_tags) ? 'selected' : ''; ?>><?php echo htmlspecialchars($tag['name']); ?></option>
                                <?php
endforeach; ?>
                            </select>
